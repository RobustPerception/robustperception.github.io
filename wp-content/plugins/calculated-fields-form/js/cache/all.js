...
return r;};lib.DATETIMESUM=function(date,format,number,to_increase){var d=_getDateObj(date,format);if(d.valid()){if(typeof number!='number'&&isNaN(parseFloat(number)))number=0;else number=parseFloat(number);if(typeof to_increase=='undefined')to_increase='d';if(/y+/i.test(to_increase))d.setFullYear(d.getFullYear()+number);else if(/d+/i.test(to_increase))d.setDate(d.getDate()+number);else if(/m+/i.test(to_increase))d.setMonth(d.getMonth()+number);else if(/h+/i.test(to_increase))d.setHours(d.getHours()+number);else if(/i+/i.test(to_increase))d.setMinutes(d.getMinutes()+number);else d.setSeconds(d.getSeconds()+number);return d;}
return false;};lib.DECIMALTOTIME=lib.decimaltotime=function(value,from_format,to_format){from_format=from_format.toLowerCase();var y=/\by+\b/i.test(to_format),m=/\bm+\b/i.test(to_format),d=/\bd+\b/i.test(to_format),h=/\bh+\b/i.test(to_format),i=/\bi+\b/i.test(to_format),s=/\bs+\b/i.test(to_format),factor=1,components={};switch(from_format)
{case'y':factor=365*24*60*60;break;case'm':factor=30*24*60*60;break;case'd':factor=24*60*60;break;case'h':factor=60*60;break;case'i':factor=60;break;}
value*=factor;if(y){components['y']=FLOOR(value/(365*24*60*60));value=value%(365*24*60*60);}
if(m){components['m']=FLOOR(value/(30*24*60*60));value=value%(30*24*60*60);}
if(d){components['d']=FLOOR(value/(24*60*60));value=value%(24*60*60);}
if(h){components['h']=FLOOR(value/(60*60));value=value%(60*60);}
if(i){components['i']=FLOOR(value/60);value=value%60;}
if(s){components['s']=value;}
...
{fbuilderjQuery['fbuilder']['extend_window'](fbuilderjQuery['fbuilder']['modules']['url']['prefix'],CF_URL);}};$.fbuilder['version']='1.1.22';$.fbuilder['controls']=$.fbuilder['controls']||{};$.fbuilder['forms']=$.fbuilder['forms']||{};$.fbuilder['htmlEncode']=function(value)
